        ...
        app.controller('CalculatorController', ['$scope', function($scope){
            ...
            $scope.calculate = function () {
                ...
                if (  $scope.current_hours > 24 ) { $scope.current_hours = 24; }
                  ...
                  var cost = $scope.current_cost / 10;

                  $scope.inc_cost = ((($scope.inc_wattage * total_hours) / 1000) * cost).toFixed(6);
                    $scope.hal_cost = ((($scope.hal_wattage * total_hours) / 1000) * cost).toFixed(6);
                      $scope.cfl_cost = ((($scope.cfl_wattage * total_hours) / 1000) * cost).toFixed(6);
                        $scope.led_cost = ((($scope.led_wattage * total_hours) / 1000) * cost).toFixed(6);
            };
              $scope.calculate();
        }]);
}) ();
